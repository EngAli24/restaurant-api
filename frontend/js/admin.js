...
document.getElementById('menu-item-form').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (response.ok) { alert('تم الإضافة!'); document.getElementById('menu-item-form').reset(); loadAdminMenuItems(); }
    } catch (error) { alert('خطأ في الاتصال'); }
});

// 💡 تم تحديث الدالة لعرض صورة مصغرة وتمرير الرابط للنافذة المنبثقة
        ...
        items.forEach(item => {
            ...
            itemsList.innerHTML += `
                <tr>
                    ...
                    <td>${catName}</td>
                    <td style="color: var(--primary-color); font-weight: bold;">${item.price} ج.م</td>
                    <td>
                        <button class="update-btn" onclick="openEditModal('${item._id}', '${safeName}', '${safeDesc}', ${item.price}, '${catId}', '${safeImage}')">تعديل ✏️</button>
...
// 💡 تم تحديث الدالة لاستقبال الصورة الحالية وعرضها
function openEditModal(id, name, desc, price, catId, imageUrl) {
...
document.getElementById('edit-item-form').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (response.ok) { alert('تم التعديل!'); closeEditModal(); loadAdminMenuItems(); }
